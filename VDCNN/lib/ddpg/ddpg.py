...
class DDPG(object):
    ...
    def select_action(self, s_t, decay_epsilon=True):
        # s_t: observation (layer_embedding)
        # if random.random() < self.epsilon:
        #     action = self.random_action()
        # else:
        # Passes observation to actor & creates action based on response
        action = to_numpy(
        ...
        # np.clip removes any values not in range [lbound, rbound] for values in action
    ...
    def load_weights(self, folder, dataset):
        if folder is None: return

        self.actor.load_state_dict(
            torch.load('{}/actor_{}.pkl'.format(folder, dataset))
        )

        self.critic.load_state_dict(
            torch.load('{}/critic_{}.pkl'.format(folder, dataset))
        )


    def save_model(self, folder, dataset):
        torch.save(
            self.actor.state_dict(),
            '{}/actor_{}.pkl'.format(folder, dataset)
        )
        torch.save(
            self.critic.state_dict(),
            '{}/critic_{}.pkl'.format(folder, dataset)
        )
