...
This code is used for the BitMask Selective Protection Technique
...
class ImportantBitsSizeEnv(ImportantBitsEnv):
    ...
    def __init__(self, model, data, data_root, compress_ratio, args, batch_size=64, num_workers=16, bitwidth=8, is_model_pruned=False, code=None):
        self.episode = 0
        super(ImportantBitsSizeEnv, self).__init__(model, data, data_root, compress_ratio, args, batch_size, num_workers, bitwidth, is_model_pruned, code)
        self.threshold = 0.5
        self.beta = 0
        
        self._get_hint()
        # Commented out because mask_hint is not a variable & the previous call to _get_hint() sets self.mask_hint
    ...
    def reward(self, acc, w_size_ratio=None):
        # self.alpha: 0.2, self.gamma: 3.0, self.beta: bound between [0.1, 1.0]
    ...
    def _cur_weight(self):
        ...
        # Adds total number of layers * min_bit, but min_bit is 0 by default
        # Returns total number of bits to protect
        cur_weight = 0.
        # quantized
        for i, action in enumerate(self.strategy):
            #action.sum is # bits protected, whereas wsize_list is num weights for layer
...
class ImportantBitsFloatSizeEnv(ImportantBitsSizeEnv):
    ...
    def __init__(self, model, data, data_root, compress_ratio, args, batch_size=64, num_workers=16, bitwidth=32, is_model_pruned=False, code=None):
        super(ImportantBitsFloatSizeEnv, self).__init__(model, data, data_root, compress_ratio, args, batch_size, num_workers, bitwidth, is_model_pruned, code)
    ...
    def step(self, action):
        ...
        self.layer_embedding[self.cur_index][-self.n_actions:] = action # Sets the last n_action indices of the new layer's embedding to the action
...
class ImportantBitsQuanSizeEnv(ImportantBitsSizeEnv):
    ...
    def __init__(self, model, data, data_root, compress_ratio, args, batch_size=64, num_workers=16, bitwidth=8, is_model_pruned=False, code=None):
        ...
        super(ImportantBitsQuanSizeEnv, self).__init__(model, data, data_root, compress_ratio, args, batch_size, num_workers, bitwidth, is_model_pruned, code)
