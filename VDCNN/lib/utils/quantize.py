...
def quantize_model(model, quantize_index, quantize_bits, quantize_bias=False, is_pruned=False):
    ...
    for i, layer in enumerate(model.modules()):
        ...
        if hasattr(layer, 'weight'):
            w = layer.weight.data
            if is_pruned:
                # Creates mask of weights that are not zero
                nz_mask = w.ne(0)
                print('*** pruned density: {:.4f}'.format(torch.sum(nz_mask) / w.numel()))
                ori_shape = w.size()
                w = w[nz_mask]
            # Sets c (bound for fixed-point weight) as the maximum absolute weight value in order to ensure all weight are representable
            c_max = w.abs().max().item() 
            if c_max > 0:
                # Calculates scaling factor as c / (2^(m-1) - 1) as per paper.
                # Note: More bits will allow higher accuracy as scaling factor can be smaller
                step = c_max / float(2 ** (n_bit[0] - 1) - 1)
                labels = torch.round(w / step)
...
def add_noise_quantize_layer(step, labels, mask, noise, prob, bitwidth):
    ...
    noise_mask_arr = (noise_mask_arr < prob).astype(cp.int8)
...
def add_noise_quantize_model(model, important_mask, noise, prob, quantize_index, quantize_bits, centroid_label_dict, is_pruned=False, code=None):
    ...
    quantize_layer_bit_dict = {n: b for n, b in zip(quantize_index, quantize_bits)}
    for i, layer in enumerate(model.modules()):
        if i not in quantize_index: # skips layers that don't have a mask provided
            continue
        ...
        if n_bit < 0:  # if -1, do not quantize
            continue
        if type(n_bit) == list:  # gives both the bit of weight and bias
            assert len(n_bit) == 2
            assert hasattr(layer, 'weight')
            assert hasattr(layer, 'bias')
        else:
        ...
        layer.weight.data = noisy_weight_data.float() # replaces weights with noisy weights
