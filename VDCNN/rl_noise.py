...
This code implements the BitMask Selective Protection Technique
...
# Torch imports
import torch
import torch.backends.cudnn as cudnnf

# Internal imports
from lib.utils.utils import prYellow, best_policy_list, export_results
from lib.env import ImportantBitsFloatSizeEnv, ImportantBitsQuanSizeEnv
...
def train(num_episode, agent, env, output, args, debug=False):
    ...
    while episode < num_episode:  # counting based on episode
        ...
        if observation is None:
            ...
            if episode > args.warmup:
                # decay. reset=True if in episode range
        ...
        # Note: reward changes based on episode, which is why diff reward comes from same policy. Also has diff acc because diff noise added each time. 
        ...
        # [optional] save intermediate model to user-specified output location
        ...
        if done:  # end of episode
            ...
            # Updating the final n_action (default to 1) positions of the observation of i + 1th record with the ith strategy policy
            for i in range(len(T) - 1):
                T[i+1][1][-env.n_actions:] = env.strategy[i] #env.strategy is action list
    ...
    best_list = best_policy_list(metric_dict['best_policy'], 'bitmask')
...
def test(num_episode, agent, env):
    '''
    Test action without having results affect the network training
...
if __name__ == "__main__":
    ...
    parser.add_argument('--max_bit', default=32, type=int, help='minimum bit to use')
    ...
    parser.add_argument('--output', default=os.path.join('..', 'checkpoints', 'checkpoint_noise'), type=str, help='output dir for drl weights and log file')
    ...
    if args.representation == 'fixed':
        env = ImportantBitsQuanSizeEnv(model, args.dataset, args.dataset_root,
        ...
        env = ImportantBitsFloatSizeEnv(model, args.dataset, args.dataset_root,
                      compress_ratio=args.preserve_ratio, num_workers=args.n_worker,
                      batch_size=args.data_bsize, args=args, bitwidth=args.bit, is_model_pruned=args.is_pruned, code=code)
    ...
    nb_actions = int(env.n_actions) # actions for weight and activation quantization
    ...
    #if args.history:
    #    print('best_outcome_history: {}'.format(metric_dict['best_history']))
        
    # Save best policy infoformation
    export_results(filename='results/drl/bitmask.csv', data=metric_dict)